...
c spectra include 38 velocity bins, 0.4 km/s wide, from -8 to 8 km/s
...
c
      ...
      common/model/profmod(2,2500,38),smod(2,2500),garea(1000)
...
c
      ...
      pstart( 1)=   5.       ! theta
      pstart( 2)=   11.      ! omega
      pstart( 3)=   0.03     ! sigma bicone wall
      pstart( 4)=   .1       ! half aperture bicone
      pstart( 5)=   0.15     ! sigma disk
      pstart( 6)=   .1       ! expansion vel. of disk
      pstart( 7)=   100.     ! expansion vel. of bicone
c mass loss rates (disk,bicone) in 10^-7 solar masses per year
      pstart( 8)=   50.      ! mass loss rate on the disk
      pstart( 9)=   .001     ! mass loss rate on the bicone
c radial dependence of temperatures (disk,bicone)
      pstart(10)=   5.       ! rotation vel. of disk at r = 1''
      pstart(11)=   300.     ! (11,13) temperature of disk
      pstart(12)=   300.     ! (12,14) temperature of bicone
      pstart(13)=   -2.      ! t=par(10+iphi)*r**(par(12+iphi))
      pstart(14)=   -2.
      do i=1,npar
...
c      call mncomd(fcn,'FIX 4',ierr,0)
...
c they go from -8 to 8 km/s in 38 steps of 0.4 km/s
...
c par(5) is the sigma of the disk measured in cphi, 0.15
      argb=0.5*(abs(cphi)-1+par(4))**2/par(3)**2
      shapeb=0.
      if(argb.le.20.)
     &     shapeb=exp(-0.5*(abs(cphi)-1+par(4))**2/par(3)**2)
      argd=0.5*(cphi)**2/par(5)**2
      shaped=0.
      if(argd.le.20.)
...
c!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &     sigmav,error(2),xdelta,beam(2)
c
c	fill array profmod in 38 bins of 0.4 km/s between -6.5 and 8 km/s
...
c
     ...
     &              .or.k.ne.k1.or.k.ne.k2)then
               ...
               profmeas(1,i+50*(j-1),k)=aa
               profmeas(2,i+50*(j-1),k)=bb
               smeas(1,i+50*(j-1))=smeas(1,i+50*(j-1))+aa
               smeas(2,i+50*(j-1))=smeas(2,i+50*(j-1))+bb
            enddo
         enddo
      enddo
...
c
      ...
      common/meas/profmeas(2,2500,38),smeas(2,2500)
      common/model/profmod(2,2500,38),smod(2,2500),garea(1000)
      call hbook1(11,'spectrum CO(3-2)meas',38.,0.,38.,0.)
      call hbook1(12,'spectrum CO(6-5)meas',38.,0.,38.,0.)
      call hbook1(13,'spectrum CO(3-2)mod',38.,0.,38.,0.)
      call hbook1(14,'spectrum CO(6-5)mod',38.,0.,38.,0.)
