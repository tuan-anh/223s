c
c  This routine is for estimating parameters like omega, theta (geometry),
c  cphi, v bicone expansion, v equatorial expansion, alpha & beta from
c  v0=(abs(cphi)/cbicone)**alpha*(vbicone-vdisk)+vdisk
c  density rho=1./4./pi/r**2*v0**beta.
c  Namely, we fit the outer source region. Disk rotation will not be considered
c  here but the inner region.
      program rrtgl_crude  
c
